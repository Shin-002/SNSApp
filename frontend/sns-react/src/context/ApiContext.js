...
const ApiContextProvider = (props) => {
    ...
    useEffect(() => {
        const getMyProfile = async() => {
            try {
                ...
                resmy.data[0] && setAskList(res.data.fillter(ask=>{return resmy.data[0].userPro === ask.askTo}))
    ...
    const deleteProfile = async() => {
        try {
            const res = await axios.post(`http://localhost:8000/api/user/profile/${profile.id}/`, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Token ${token}`
                }
            })
            setProfiles(profiles.fillter(dev => dev.id !== profile.id))
            setProfile([])
    ...
    const changeApprovalRequest = async(uploadDataAsk, ask) => {
        try {
            const res = await axios.put(`http://localhost:8000/api/dm/approval/${ask.id}/`, uploadDataAsk, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Token ${token}`
                }
